...
function App() {
  ...
  const handleTimer = (i) => {
    if (status !== "RUNNING" && !isBreak) {
      ...
      if (!isBreak) setCurrentInitialTime(newTime); // Only update initial time if not on break
    }
  };
  ...
  const handleBreak = (i) => {
    if (status !== "RUNNING" && isBreak) {
      ...
      if (isBreak) setCurrentInitialTime(newBreak); // Only update initial time if on break
    }
  };
